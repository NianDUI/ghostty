...
- 合并上游时若 diff 触及 `macos/Sources/Ghostty/Surface View/SurfaceView.swift` 的 `SurfaceSearchOverlay`（搜索条拖动 / snap-to-corner / 落位对齐 alignment / `padding`），必须回核 XGhostty 的 `snappedFrame(for:in:barSize:)`——它是对上游搜索条落位的**手算镜像**（fork 侵入点：`onBarFrameChange` 可选回调 + `snappedFrame`，给座舱独立 `NSHostingView` 重建命中用，默认 nil、主 app 零影响）。上游改落位算法时这里会**静默算错**（不一定产生 merge 冲突）、致座舱 ⌘F 搜索框「拖动后点不动」复发。选型依据与彻底解（方案 A）见 `docs/plan/xghostty.md` 第四十二批「方案选型」。另：座舱搜索框「切 tab 跟随当前会话」靠**复用同一 `SearchOverlayHostingView` 实例、只换 `rootView`** 保留拖动落位（`SurfaceSearchOverlay` 内部 `@State corner`）；上游若把 `corner` / 拖动状态外置或重构 `SurfaceSearchOverlay` 的 `@State` 结构，座舱切 tab 后搜索框会**静默跳回右上角**（不报错），须回核 `migrateSearchOverlay`（`XGhosttyConsole.swift`）。见第四十三批。再另：上游 `endSearch()` override（`SurfaceView_AppKit.swift`，commit `97c5a21ab`）内含 `Ghostty.moveFocus(to: self)`（**延迟 50ms 的 makeFirstResponder**）——对座舱的隐藏旧 surface 触发会把焦点拉回旧 tab、打字进旧 pty（「a→b 输入进 a」滞后一格，静默无报错）。且**手动直清 `searchState = nil` 绕不开**：didSet 发 `end_search` → core（`src/Surface.zig` `.end_search`）**无条件回发** apprt action → `Ghostty.App.swift` handler 异步再调 `surfaceView.endSearch()`。防线是 `XGhosttySurfaceView` 子类按 `isHiddenOrHasHiddenAncestor` 拦截（`XGhosttyConsole.swift`）；座舱 surface **必须**经该子类创建。上游改 `endSearch` override / core `end_search` 回发逻辑 / `Ghostty.App` 的 endSearch handler 时须回核拦截仍生效。见第四十五批。
- 合并上游时若 diff 触及 `src/terminal/formatter.zig` 的 `PageFormatter.formatWithState`：fork 在其中加了 `skip_pad_x` —— 复制 CJK 软换行命令时跳过 shell（zsh/readline）在宽字符放不下行尾时填充的 narrow 空格（判据：本行 `wrap` 且行尾 narrow 空格、下一行首 cell 为 wide，独立于 `trim`）。这是对上游共享文件的功能性修改（非默认 nil 的侵入点），上游若改动该函数会产生**正常 merge 冲突**（非静默失效），届时回核 `skip_pad_x` 是否仍贴合 unwrap / `blank_cells` 流程、`trim=false` 路径（macOS 复制经 `dumpTextLocked`）是否仍命中。见第四十四批。
