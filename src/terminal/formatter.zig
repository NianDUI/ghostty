    ...
    fn formatWithStateEmit(
        ...
        for (start_y..end_y + 1) |y_usize| {
            ...
            while (cell_i < cells_subset.len) : (cell_i += 1) {
                ...
                // Skip before the fast path so a simple narrow space isn't
                // consumed as part of a batched cell run.
                ...
                // A pad-space row is deliberately kept on the per-cell path:
                // a run that starts before the pad could otherwise batch over
                // it before the loop reaches skip_pad_x.
                if (cp_map_empty and skip_pad_x == null) fast: {
...
test "selection CJK pad-space soft-wrap drops pad" {
    ...
    const alloc = testing.allocator;
    const io = testing.io;
    var t = try Terminal.init(io, alloc, .{ .cols = 11, .rows = 24 });
    defer t.deinit(alloc);
    var s = t.vtStream();
    defer s.deinit();
...
test "selection ASCII space at soft-wrap boundary kept" {
    ...
    const alloc = testing.allocator;
    const io = testing.io;
    var t = try Terminal.init(io, alloc, .{ .cols = 6, .rows = 24 });
    defer t.deinit(alloc);
    var s = t.vtStream();
    defer s.deinit();
